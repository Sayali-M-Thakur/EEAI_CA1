...
#     df[Config.TICKET_SUMMARY] = df[Config.TICKET_SUMMARY].values.astype('U')
#
#     # Note: We are not assigning "y" to "y2" here. This will be done dynamically in Main.py.
#     return df
#
#
# def de_duplication(data: pd.DataFrame):
...
#     df[Config.INTERACTION_CONTENT] = df[Config.INTERACTION_CONTENT].replace(r'\s+', ' ', regex=True).str.strip()
#
#     # Ensure y1 is filtered based on its frequency
#     good_y1 = df.y1.value_counts()[df.y1.value_counts() > 10].index
#     df = df.loc[df.y1.isin(good_y1)]
#     return df
#
...
def de_duplication(data: pd.DataFrame):
    ...
    # Existing de-duplication code
    # ...

    # Ensure no missing values in 'ic_deduplicated'
    data = data.fillna('')
